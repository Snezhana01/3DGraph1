Surfaces.prototype.parCylinder = (count = 10, point = new Point(0, 0, 0), color = null) => {
	const points = [];
	const edges = [];
    const polygons = [];
    const p = 5;
    ...
    if(color == null){
        for(let i = 0; i <= points.length; i++){
            if(i + 2 * count + 2 < points.length && ((i + 1) % (2 * count + 1) != 0)){
                var color1 = '#' + Math.random().toString(16).slice(3, 9);
                polygons.push(new Polygon([i, i + 1, i + 2 * count + 2, i + 2 * count + 1], color1));
            }
        }
    }
